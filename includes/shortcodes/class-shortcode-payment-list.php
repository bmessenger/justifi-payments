<?php
/**
 * Add the Shortcode to display the Payment List for an Organization
 *
...
class Payment_List {
	...
	public function generate_shortcode() {
		add_shortcode( 'Justifi_Payment_List', array( $this, 'form_generate' ) );
	}
	...
	public function form_generate () {
		...
		if ( $biz['subaccount_id'] ) {
			...
			$output = '<div class="justifi-table"><justifi-payments-list account-id="' .$biz['subaccount_id']. '" auth-token="' .$biz['web_token']. '" columns="created_at"></justifi-payments-list><div class="payment-details"></div></div>';
			$output .= '
			<script type="text/javascript">
			  (function() {
				...
				paymentList.addEventListener("payment-row-clicked", (event) => {
				  const entityID = event.detail.id;
			  	  let output =  \'<justifi-payment-details payment-id="\' + entityID + \'" auth-token="\' + token + \'"></justifi-payment-details><span id="close-details"><i class="fas fa-times"></i></span>\';
			...
			$output = '
				<div class="message no-account">
					<p>Payment list unavailable.  Please ensure you have access to view this information.</p>
				</div>
			';
...
new \Justifi_Payments\Shortcodes\Payment_List();